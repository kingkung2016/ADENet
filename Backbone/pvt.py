...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            ...
            setattr(self, f"pos_embed_new{i + 1}", pos_embed)
        ...
        # init weights
        for i in range(num_stages):
            pos_embed = getattr(self, f"pos_embed_new{i + 1}")
...
if __name__== '__main__':
    ...
    # 定义模型
    pvt_net = pvt_small(pretrained=True).cuda()
    # 加载模型权重参数
    pvt_pretrained_path = '../pretrained/pvt_small.pth'
    pvt_pretrained_dict = torch.load(pvt_pretrained_path)

    model_dict = pvt_net.state_dict()
    pretrained_dict = {k: v for k, v in pvt_pretrained_dict.items() if k in model_dict}
    model_dict.update(pretrained_dict)
    pvt_net.load_state_dict(model_dict)

    output = pvt_net(input)
    #转化张量
