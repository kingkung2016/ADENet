...
if __name__== '__main__':
    ...
    transformer_net = SwinTransformer(embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24]).cuda()
    # 加载模型权重参数
    swin_pretrained_path = '../pretrained/swin_tiny_patch4_window7_224_22k.pth'
    swin_pretrained_dict = torch.load(swin_pretrained_path)

    model_dict = transformer_net.state_dict()
    pretrained_dict = {k: v for k, v in swin_pretrained_dict.items() if k in model_dict}
    model_dict.update(pretrained_dict)
    transformer_net.load_state_dict(model_dict)

    output = transformer_net(input)
    #转化张量
