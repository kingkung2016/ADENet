...
#加载网络
#from models.Network import Architecture
...
def test(test_root, model_fusion, salmap_root, image_size, cuda):
    ...
    for id, (data, depth, img_name, img_size) in enumerate(test_loader):
        ...
        #生成测试结果
        F_out, _, _, _ = model_fusion(inputs, depth)
        ...
        #保存所有阶段图片
        # _, F2_out, F3_out, F4_out = model_fusion(inputs, depth)
        #
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', type=str, default='./checkpoint/RGBD-SOD_iter_R.pth', help='num of checkpoints')
    parser.add_argument('--test-root', type=str, default='F:/dataset/RGB-D/test_dataset/', help='path to the test dataset')
    parser.add_argument('--salmap-root', type=str, default='./sal_map/', help='path to saliency map')
    parser.add_argument('--cuda', type=bool, default=True, help='use cuda')
    args = parser.parse_args()

    #定义测试集
    test_data = ['DES-RGBD135', 'NLPR', 'DUT-RGBD', 'NJUD', 'STERE-797', 'STERE-1000', 'SSD', 'LFSD', 'SIP', 'ReDWeb-S']

    # 加载三个网络
    RGBD_model = Architecture()
    # 加载权重文件
    RGBD_model.load_state_dict(torch.load(args.ckpt, map_location='cuda:0'))
    ...
    for test_name in test_data:
        ...
        test(test_root, RGBD_model, save_salmap_root, args.img_size, args.cuda)
