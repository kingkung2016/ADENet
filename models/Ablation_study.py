...
class CMF_ablation(nn.Module):
    def __init__(self, in_channel, out_channel, reduction=16):
        super(CMF_ablation, self).__init__()
...
# PMA 去掉CLA模块
class PMA_ablation_1(nn.Module):
    def __init__(self, in_channels=[64, 128, 256, 512]):
        super(PMA_ablation_1, self).__init__()
        self.image_size = 256
        self.upsamplex2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)

        self.conv_dilation = ASPP(in_channels[3], in_channels[2], rates=[1,2,3,4])

        self.Group_Conv_3 = MC(in_channels[2], in_channels[1])
        self.Group_Conv_2 = MC(in_channels[1], in_channels[0])
        self.Group_Conv_1 = MC(in_channels[0], in_channels[0])


        self.Sal_Head_1 = Sal_Head(in_channels[0])
        self.Sal_Head_2 = Sal_Head(in_channels[0])
        self.Sal_Head_3 = Sal_Head(in_channels[1])
        self.Sal_Head_4 = Sal_Head(in_channels[2])


    def forward(self, fuse1, fuse2, fuse3, fuse4):

        fusion_4 = self.conv_dilation(self.upsamplex2(fuse4))

        fusion_43 = fusion_4 + fuse3
        f_out3 = self.Group_Conv_3(fusion_43)
        fusion_32 = self.upsamplex2(f_out3) + fuse2
        f_out2 = self.Group_Conv_2(fusion_32)
        fusion_21 = self.upsamplex2(f_out2) + fuse1
...
# PMA 去掉MC模块
class PMA_ablation_2(nn.Module):
    def __init__(self, in_channels=[64, 128, 256, 512]):
        super(PMA_ablation_2, self).__init__()
        self.image_size = 256
        self.upsamplex2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        ...
        self.Group_Conv_3 = nn.Sequential(
            BaseConv2d(in_channels[3], in_channels[2], kernel_size=1, stride=1, padding=0)
        )
        self.Group_Conv_2 = nn.Sequential(
            BaseConv2d(in_channels[2], in_channels[1], kernel_size=1, stride=1, padding=0)
        )
        self.Group_Conv_1 = nn.Sequential(
            BaseConv2d(in_channels[1], in_channels[0], kernel_size=1, stride=1, padding=0)
        )
...
# 全部去掉
class PMA_ablation_3(nn.Module):
    def __init__(self, in_channels=[64, 128, 256, 512]):
        super(PMA_ablation_3, self).__init__()
        self.image_size = 256
        self.upsamplex2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
    ...
    def forward(self, fuse1, fuse2, fuse3, fuse4):
        ...
        fusion_43 = self.Group_Conv_3(fuse4) + fuse3
        fusion_32 = self.Group_Conv_2(fusion_43) + fuse2
        fusion_21 = self.Group_Conv_1(fusion_32) + fuse1


        F4_out = F.interpolate(self.Sal_Head_4(fuse4), self.image_size, mode='bilinear', align_corners=True)
        F3_out = F.interpolate(self.Sal_Head_3(fusion_43), self.image_size, mode='bilinear', align_corners=True)
        F2_out = F.interpolate(self.Sal_Head_2(fusion_32), self.image_size, mode='bilinear', align_corners=True)
        F1_out = F.interpolate(self.Sal_Head_1(fusion_21), self.image_size, mode='bilinear', align_corners=True)

        return F1_out, F2_out, F3_out, F4_out


# 只使用ASPP
class PMA_ablation_4(nn.Module):
    def __init__(self, in_channels=[64, 128, 256, 512]):
        super(PMA_ablation_4, self).__init__()
        self.image_size = 256
        self.upsamplex2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
    ...
    def forward(self, fuse1, fuse2, fuse3, fuse4):
        fuse4 = self.conv_dilation(fuse4)
...
if __name__== '__main__':
    ...
    # test_module_1 = PMA_decoder(in_channels).cuda()
    #
    # out4321, out432, out43, feature_4321 = test_module_1(fusion_features[0], fusion_features[1], fusion_features[2], fusion_features[3])
